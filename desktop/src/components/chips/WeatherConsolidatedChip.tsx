...
export default function WeatherConsolidatedChip({
  items,
  comfort,
  colorMode = "channel",
  pinned = false,
  ...
  return (
    ...
    >
      ...
      {comfort && (
        <div className={clsx("flex items-center text-[10px] min-h-4", c.textFaint)}>
          {items.map((item, i) => (
            item.detail ? (
              <div key={item.id} className="flex items-center">
