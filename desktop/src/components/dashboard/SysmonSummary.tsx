...
function UsageBar({ label, pct }: { label: string; pct: number }) {
  return (
    <div className="flex items-center gap-2">
      <span className="text-[10px] text-fg-3 w-8 shrink-0">{label}</span>
...
export default function SysmonSummary({ prefs }: SysmonSummaryProps) {
  ...
  return (
    <div className="space-y-2">
      {prefs.cpu && <UsageBar label="CPU" pct={cpuPct} />}
      {prefs.ram && <UsageBar label="RAM" pct={memPct} />}
      {prefs.gpu && gpuPct !== null && <UsageBar label="GPU" pct={gpuPct} />}
