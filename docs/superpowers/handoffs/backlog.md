...
### Pending
- [ ] **Tag and push `desktop-v1.0.12`.** All eight tonight-of-2026-05-10 PRs are merged on `main` (HEAD = `4eecc71`). The version strings are bumped (#159) but the git tag isn't pushed. Pushing it triggers `desktop-release.yml` to build cross-platform installers + the website auto-rebuild for the new download link. Single command: `git tag desktop-v1.0.12 && git push origin desktop-v1.0.12`. Wait for user go-ahead.
