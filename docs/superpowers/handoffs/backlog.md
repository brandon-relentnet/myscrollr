...
### In Progress
- [ ] **Ticker monitor picker** — design phase complete, pending implementation. Spec at `docs/superpowers/specs/2026-05-10-ticker-monitor-picker-design.md` (committed `7a90cc1`). Single-monitor selection with stable-fingerprint identity and primary fallback. ~80–120 LOC across 7 files (Rust commands + JS settings UI + prefs schema + tests). Designed so the `tickerMonitorId: string | null` schema extends to `tickerMonitorIds: string[]` later if/when one-ticker-per-monitor (Approach A) becomes worth the bigger investment. Compositor adapters need no changes — Hyprland / Sway / KDE/KWin already accept absolute cross-monitor coords. Spec self-review passed; user reviewing before writing-plans is invoked.
...
- [ ] **Triage open PR #104** ("Feature/favorite team selection" by Enanimate). 5 weeks old, no description, +521/-164 across 13 files, `mergeStateStatus: UNKNOWN`. Almost certainly conflicts with tonight's Fantasy/Sports work. Either rebase + review, or close.
